...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (heroVideoCanvas && typeof leapData !== 'undefined') {
    ...
    // Frame the scroll-driven cover settles on (0-based, so frame 115 = 114).
    // Mobile runs the cover a little further, settling on frame 136 (= 135).
    const endFrame    = window.matchMedia('(max-width: 768px)').matches ? 135 : 114;

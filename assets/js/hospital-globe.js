...
(function () {
  ...
  function init() {
    ...
    /* Teal state borders via GeoJSON — light/subtle */
    ...
    HOSPITALS.forEach(function(h) {
      ...
      var marker = L.marker([h.lat, h.lng], { icon: icon })
        .bindTooltip(
          '<div class="hcm-tooltip">'
            + '<span class="hcm-tooltip__city">' + h.city + ', ' + h.state + '</span>'
            + '<span class="hcm-tooltip__meta">' + h.cases + ' cases &middot; ' + (h.type === 'consignment' ? 'Consignment' : 'Stock & Bill') + '</span>'
          + '</div>',
          { className: 'hcm-tooltip-wrap', direction: 'top', offset: [0, -4], sticky: false }
        )
        .addTo(map);
